...
There's some unrest in the minion ranks: minions with ID
numbers like "1", "42", and other "good" numbers have been lording it
over the poor minions who are stuck with more boring IDs. To quell the
...
from a hat. That number is the starting index in that string of
primes, and the minion's new ID number will be the next five digits in
...
cases. Note that it may also be run against hidden test cases not
shown here.
